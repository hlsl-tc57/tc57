...
smallest containing scope of a \texttt{S} is the \textit{parent scope} of
...
function declaration, the the name is declared to be a \textit{class-name} in
...
\p The point of declaration of a template parameter is immediately after it's
...
\ref{Stmt.For}, \ref{Stmt.While}), or a substatement such a statement.
...
the translation unit. Each \textit{using-declaration} nominating a namespace
...
\textit{using-declaration} following the member's point of declaration.
...
\}}) enclosing it's optional \textit{member-specialization}. Declarations within
...
It's potential scope begins at its point of declaration and terminates at the
end of the enumerator's declaration.
...
\textit{template-parameter}. A \textit{template-declaration}'s introduced scope
begins at its \textit{template-parameter-list} and extends to the end of the
