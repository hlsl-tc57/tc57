...
\texttt{vector<T,x>}. The destination value is the value in the source vector
replicated into each element of the destination.

\Sec{Matrix splat conversion}{Conv.msplat} \p A glvalue of type \texttt{T} can
be converted to a cxvalue of type \texttt{matrix<T,x,y>} or a prvalue of type
\texttt{T} can be converted to a prvalue of type \texttt{matrix<T,x,y>}. The
